...
async def get_all_users(db: Session = Depends(get_db), skip: int = 0,
                        limit: int = Query(10, ge=10, le=100, description="Limit of users to get")):
    print(skip, limit)
...
async def get_user(user_id: int = Path(ge=1, description="ID of the user"), db: Session = Depends(get_db)):
...
@router.post("/", status_code=status.HTTP_201_CREATED,
             response_model=UserResponse)  # change return code to make post method idempotent
