from sqlalchemy.orm import Session
from src.models import Note
from src.schemas.notes import NoteBase, NoteUpdate


async def get_all_notes(db: Session):
    all_notes = db.query(Note).all()
    return all_notes


async def get_note(db: Session, note_id: int):
    note = db.query(Note).filter(Note.id == note_id).first()
    return note


async def create_note(db: Session, note: NoteBase):
    new_note = Note(title=note.title, description=note.description)
    db.add(new_note)
    db.commit()
    db.refresh(new_note)
    return new_note


async def update_note(db: Session, note_id: int, note_update: NoteUpdate):
    note = db.query(Note).filter(Note.id == note_id).first()
    if note:
        pass
    return note


async def delete_note(db: Session, note_id: int):
    return {}
